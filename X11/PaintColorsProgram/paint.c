...
// functions 
static void create_color(XftColor *color_red, const char *name);
...
int main()
{
    ...
    Window main_win;
    GC gc;
    XftColor *color_red = malloc(sizeof(XftColor));
    ...
    create_color(color_red, RED);
    gc = create_gc(LINE, color_red);
    ...
    XFreeGC(dpy, gc);
    XCloseDisplay(dpy);
    XftColorFree(dpy, vis, DefaultColormap(dpy, scr), color_red);
...
static void create_color(XftColor *color_red, const char *name)
{
    if (!XftColorAllocName(dpy, vis, DefaultColormap(dpy, scr), name, color_red))
        errx(1, "Cant allocate color");

    color_red->pixel |= 0xff << 24;
}
